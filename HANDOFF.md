...
|------|-----|-----|-----|
| 15s 英文 | 0.44x | **24.00x** ✅ | ~7.5x |
| 30s 中文 | 0.25x | **20.70x** ✅ | ~7.5x |
| 90s 英文 | — | **18.87x** ✅ | — |
| 90s 日文 | — | **20.45x** ✅ | — |
| 180s 英文 | — | **17.34x** ✅ | — |
| 180s 中文 | — | **15.87x** ✅ | — |
...
> **关键拐点**：从 0.25x → 20.70x 是约 **83x 提速**，0.6B 全部测试 ≥ 15x，**RTFx 是 candle CUDA 的 2.5-3.2x**。
...
| **GPU-resident next-token + adaptive split chunk** | **23.67x** | **20.33x** | **17.09x** | argmax_into_slot + embed_lookup_single_i32 让 token 不离 GPU；fused_gqa_decode_split chunk_size 改成 cur_len<2048 用 256, ≥2048 用 512 |
| **fused QKV extract kernel** | **23.98x** | **20.61x** | **17.30x** | 把 qkv_extract_q_norm_rotary + qkv_extract_kv_norm_rotary_cache 合并成一个 launch（grid.y 区分 Q vs K+V head slot），28×N_step 个 launch 省一半 |
